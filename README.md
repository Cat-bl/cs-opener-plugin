...
> **CS:GO 模拟开箱** TRSS-Yunzai 插件 — 商城浏览 / 视频开箱 / 仓库管理 全套体验
...
**金币循环**：初始 10000 + 每日 `#csgo 签到` +500 + `#csgo 出售 [uid]` 卖物品。开箱单价见箱子（一般 17）。

| 命令 | 说明 | 输出 |
|---|---|---|
| `#csgo` 或 `#csgo 帮助` | 命令清单 | 图 |
| `#csgo 签到` | **每日领 500 金币**（CST 0 点重置） | 文字 |
| `#csgo 商城` | 默认看武器箱 | 图 |
| `#csgo 商城 印花胶囊` | 切换类别<br>（武器箱 / 纪念包 / 高光纪念包 / 印花胶囊 / 签名胶囊 / 布章包 / 胸章胶囊 / 涂鸦箱 / 音乐盒集） | 图 |
| `#csgo 看 反冲武器箱` | 查看单个箱子详情（含全部物品+概率） | 图 |
| **`#csgo 开箱`** | **开你的默认箱**（不带箱名 = 默认；从未设置 = 第一个武器箱） | **MP4 视频，约 14s** |
| `#csgo 开箱 反冲武器箱` | 直接开指定箱子，**并记忆为默认** | MP4 |
| `#csgo 选箱 反冲武器箱` | 只切换默认箱不开箱（不花钱）| 文字 |
| `#csgo 选箱` | 查看当前默认箱 | 文字 |
| `#csgo 仓库` | 自己全部库存 | 图 |
| `#csgo 仓库 红` | 按品质筛选（白 / 浅蓝 / 蓝 / 紫 / 粉 / 红 / 金 / 暗金） | 图 |
| `#csgo 出售 abc123` | 出售单件（uid 至少前 4 位，仓库图每件下方有 6 位标识） | 文字 |
| `#csgo 出售 全部` | 一键卖光所有库存 | 文字 |
| `#csgo 出售 全部 蓝` | 只卖该品质（白/浅蓝/蓝/紫/粉/红/金） | 文字 |
| `#csgo 记录` | 开箱历史 + 品质分布柱状图 + 最稀有掉落 | 图 |
| `#csgo 概率` | 查看当前全局概率（所有人统一） | 文字 |
| `#csgo 重置存档` | 清空自己的金币/库存/记录（60s 内连发 2 次确认） | 文字 |
...
| `#csgo 状态` | 查看下载任务实时进度 |
| `#csgo更新` | 从 git 仓库拉最新插件代码（拉完需重启 Yunzai 生效） |
| `#csgo 概率预设 欧皇` | 切全局预设（默认/欧皇/极品/均匀/残酷），写入 `config.yaml` |
| `#csgo 设置概率 红 1000` | 调全局单档（万分比），写入 `config.yaml` |
...
| 报「数据未初始化」 | 还没下载图片，主人发 `#csgo 更新数据`（约 5–60 分钟） |
