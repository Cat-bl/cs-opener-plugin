 ...
 *
 *   #cs 更新数据         触发增量下载（后台执行 + 节流进度回执，完成自动 reload）
...
export class CsgoAdmin extends plugin {
  constructor() {
    super({
      ...
      priority: 5000,
      rule: [
        { reg: '^#?\\s*cs\\s*更新数据(\\s+强制)?$', fnc: 'updateData' },
        { reg: '^#?\\s*cs\\s*状态$',                 fnc: 'status' },
      ],
    })
  }
  ...
  async updateData(e) {
    ...
    if (running) {
      await e.reply(`已有下载任务在跑（开始于 ${Math.round((Date.now()-running.startAt)/1000)} 秒前），用 #cs 状态 查看`)
      return true
    }
